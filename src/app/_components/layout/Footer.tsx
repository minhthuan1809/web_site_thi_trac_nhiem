...
export default function Footer() {
  ...
  if (!quickLink?.Information?.length && !contactOne?.length && !contactTwo?.length && !logo?.length && !copyright?.length) {
    return [];
  }
          ...
          <div className="col-span-2 lg:col-span-1">
            ...
            <ul className="grid grid-cols-2 lg:grid-cols-1 gap-2 text-sm lg:text-base text-gray-400">
              {quickLink?.Information?.map((item: any, index: number) => (
                <li key={index}>
                  <Link href={item.url} target={item.target} className="hover:text-white transition-colors">
                    {item.title}
                  </Link>
                  </li>
              ))}
